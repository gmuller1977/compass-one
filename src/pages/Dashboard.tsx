...
const lancamentos = [
  ...
  { descricao: 'Salário', categoria: 'Entrada', valor: 11548.00, data: 'Ontem' },
    ...
    <div style={{ minHeight: '100vh', background: COR.fundo, fontFamily: "-apple-system,'Inter',sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, textTransform: 'capitalize' }}>{hoje}</span>
          <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff', fontSize: 14, fontWeight: 600 }}>G</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
          {resumo.map(item => (
            <div key={item.label} style={{ background: COR.branco, borderRadius: 14, padding: '20px 22px', border: `1px solid ${COR.borda}` }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 700, color: COR.texto, letterSpacing: -0.5 }}>{item.valor}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div style={{ background: COR.branco, borderRadius: 14, padding: '22px 24px', border: `1px solid ${COR.borda}` }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h2 style={{ fontSize: 15, fontWeight: 600, color: COR.texto, margin: 0 }}>Orçamento do mês</h2>
              <span style={{ fontSize: 12, color: COR.azul, cursor: 'pointer' }}>Ver tudo</span>
            </div>
            {categorias.map(cat => (
              <div key={cat.nome} style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                  ...
                  <span style={{ fontSize: 12, color: COR.textoSuave }}>
                    {formatarValor(cat.gasto)} <span style={{ color: COR.borda }}>/ {formatarValor(cat.limite)}</span>
                  </span>
                </div>
              ...
              <div key={i} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: COR.textoSuave, marginTop: 2 }}>{l.categoria} · {l.data}</div>
                  </div>
                </div>
                <div style={{ fontSize: 14, fontWeight: 600, color: l.valor > 0 ? COR.verde : COR.texto }}>
                  {l.valor > 0 ? '+' : ''}{formatarValor(l.valor)}
                </div>
              </div>
            ))}
          </div>
